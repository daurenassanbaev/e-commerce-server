...
import com.ecommerce.userservice.config.SecurityProperties;
import com.ecommerce.userservice.exception.LogoutException;
import com.ecommerce.userservice.exception.TokenRefreshException;
import com.ecommerce.userservice.exception.UserLoginException;
...
import com.ecommerce.userservice.model.dto.response.AdminTokenResponseDto;
import com.ecommerce.userservice.model.dto.response.JwtAuthenticationResponseDto;
import com.ecommerce.userservice.model.dto.response.KeycloakRoleResponseDto;
import com.ecommerce.userservice.model.enums.UserRole;
import com.ecommerce.userservice.util.KeycloakAdminTokenProvider;
import com.ecommerce.userservice.util.KeycloakRoleProvider;
    ...
    public JwtAuthenticationResponseDto createUser(RegistrationRequestDto dto) {
        ...
        String userId = keycloakWebClient.post()
                ...
                .exchangeToMono(response -> {
                   ...
                   return Mono.error(new UserRegistrationException("User creation failed or Location header missing"));
                })
                .block();
        ...
        return keycloakWebClient.post()
                ...
                .onStatus(
                        ...
                        response -> response.bodyToMono(String.class)
                                .map(body -> new UserLoginException(
    ...
    public void updateUser(UUID keycloakId, UpdateUserRequestDto dto) {
        ...
        keycloakWebClient.put()
                ...
                .onStatus(
                        status -> status.is4xxClientError() || status.is5xxServerError(),
                        response -> response.bodyToMono(String.class)
                                .map(body -> new UserRegistrationException("Keycloak update failed: " + response.statusCode() + " - " + body))
                )
                .toBodilessEntity()
                .block();
    }
    ...
    public void deactivateUser(UUID keycloakId) {
        ...
        keycloakWebClient.put()
                ...
                .onStatus(
                        status -> status.is4xxClientError() || status.is5xxServerError(),
                        response -> response.bodyToMono(String.class)
                                .map(body -> new UserRegistrationException("Keycloak deactivation failed: " + response.statusCode() + " - " + body))
                )
                .toBodilessEntity()
                .block();
    }
    ...
    public void activateUser(UUID keycloakId) {
        ...
        keycloakWebClient.put()
                ...
                .onStatus(
                        status -> status.is4xxClientError() || status.is5xxServerError(),
                        response -> response.bodyToMono(String.class)
                                .map(body -> new UserRegistrationException("Keycloak activation failed: " + response.statusCode() + " - " + body))
                )
                .toBodilessEntity()
                .block();
    }
